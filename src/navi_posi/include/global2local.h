...
vec2d GPS_to_local_coordinate(vec2d origin_gps, vec2d obj_gps, double veh_path_angle) {
  ...
  //latitude对应vec2d中的lat，longitude对应vec2d中的log
  ...
  double b = rad(origin_gps.log) - rad(obj_gps.log);
  double s = 2 * asin(sqrt(pow(sin(a / 2), 2) +
                           cos(radLat1)*cos(radLat2)*pow(sin(b / 2), 2)));
  s = s * EARTH_RADIUS;
  s = round(s * 10000) / 10000;
  //计算obj_gps相对于origin_gps航向角
  double x = sin(obj_gps.log - origin_gps.log) * cos(obj_gps.lat);
  double y = cos(origin_gps.lat)*sin(obj_gps.lat)
      - sin(origin_gps.lat)*cos(obj_gps.lat)*cos(obj_gps.log - origin_gps.log);
  double bearing;
  if (x >=  0 && y >= 0) {
    bearing = atan2(x, y);
  }
