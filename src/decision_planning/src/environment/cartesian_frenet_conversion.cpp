...
double NormalizeAngle(const double angle)
{
	double angle_out;
	if (angle > pi) {
		angle_out = angle - floor((angle + pi) / (2 * pi))*pi;
	}
	else if (angle < -pi) {
		angle_out = angle + floor(abs((angle - pi)) / (2 * pi))*pi;
	}
	else {
		angle_out = angle;
	}
...
void cartesian_to_frenet(
	...
	array<double, 3>* const ptr_s_condition,
	array<double, 3>* const ptr_d_condition) {
	...
	ptr_d_condition->at(0) =
		copysign(sqrt(dx * dx + dy * dy), cross_rd_nd);
...
void frenet_to_cartesian(
	...
	const array<double, 3>& s_condition,
	const array<double, 3>& d_condition, double* const ptr_x,
	...
	*ptr_v = sqrt(one_minus_kappa_r_d * one_minus_kappa_r_d *
...
double CalculateTheta(const double rtheta,
	const double rkappa,
	const double l,
	const double dl) {
...
double CalculateKappa(const double rkappa,
...
const Vec2d& rpoint,
const double l) {
const double x = rpoint.x() - l * sin(rtheta);
const double y = rpoint.y() + l * cos(rtheta);
return Vec2d(x, y);
}*/

double CalculateLateralDerivative(
	const double rtheta, const double theta, const double l,
	const double rkappa) {
	return (1 - rkappa * l) * tan(theta - rtheta);
}

double CalculateLongitudinalDerivative(
	const double rtheta, const double theta, const double l,
	const double rkappa,const double veh_spd_cur) {
	return veh_spd_cur*(1 - rkappa * l) / cos(theta - rtheta);
}
...
double CalculateSecondOrderLateralDerivative(
	...
	const double cos_theta_diff = cos(theta_diff);
	const double res = -(rdkappa * l + rkappa * dl) * tan(theta - rtheta) +
