...
void obs_handle(const kalman::radar_mobileye_data_multi msg){
  ...
  for(uint8_t i=0;i<num;i++){
    ...
    int type = 1;
    int width = max(float(1),msg.objs[i].fusion_Width);
    int height = max(float(1),msg.objs[i].fusion_Length);
    //if (abs(vz_temp)+abs(vx_temp)>=0){
      obstacle obs_temp = obstacle((double)z_temp, (double)x_temp,
                                   (double)vz_temp, (double)vx_temp, width, height, type);
      obs_info.insert(obs_info.begin() + obs_num_select, obs_temp);
      obs_num_select=obs_num_select+1;
    //}
  }
}
...
void navi_handle(const navi_posi::navi_info msg){
  ...
  navi_test.Next_RelX.clear();//下一路段坐标点串x
  navi_test.Next_RelY.clear();//下一路段坐标点串y
  int num=msg.RelX.size();
  for(uint8_t i=0;i<num;i++){
    navi_test.RelX.push_back(msg.RelX[i]);
    navi_test.RelY.push_back(msg.RelY[i]);
  }
  num=msg.Next_RelX.size();
  for(uint8_t i=0;i<num;i++){
    navi_test.Next_RelX.push_back(msg.Next_RelX[i]);
    navi_test.Next_RelY.push_back(msg.Next_RelY[i]);
  }
