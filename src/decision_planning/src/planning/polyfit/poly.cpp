...
void poly::getpoints(std::vector<double> x,std::vector<double> y)
{
    coord[0].clear();
    coord[1].clear();
...
void poly::setdegree(int deg_in){
    degree=deg_in;
}

void poly::inter(int deg_in, std::vector<double> x, std::vector<double> y)
{   
    poly::setdegree(deg_in);
    poly::getpoints(x, y);
