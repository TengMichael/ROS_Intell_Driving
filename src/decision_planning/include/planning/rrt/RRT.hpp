...
namespace rrt
{
	...
	public:
        ...
        virtual bool plan(vector<obstacle> obs_info, int direction);
	...
	private:
        ...
        bool checkPoint(Utils::Point<T> pt,vector<obstacle> obs_info);
        Utils::Point<T> generatePoint(int);
        Utils::Point<T> generateBiasedPoint(int,int);
        void growTree(Utils::Point<T>,int);
        std::pair<Utils::Point<T>, int>  findClosestNode(Utils::Point<T>,int);
