...
#define DistY_threshold 5//2.5m for 20hz, 5m for 10hz
...
void Radar208_dataprocess2(void){
  uint8_t k=0;
  static uint8_t Obj208_ID_Total_last[Sensors_208Num]={0};
  ...
  memcpy(&(objs208_temp),&(objs208_out),sizeof(objs208_out));

  for(uint8_t s=0;s<Sensors_208Num;s++){
    k=0;
    for(uint8_t i=0;i<Obj208_ID_Total[s];i++){
      for(uint8_t j=0;j<Obj208_ID_Total_last[s];j++)
      {
    ...
    Obj208_ID_Total_last[s]=Obj208_ID_Total[s];
    Obj208_ID_Total[s]=k;
  }
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub1 = nh.subscribe ("can_device0_can2_receive", 1000,Radar208_extract);
  ros::Subscriber sub2 = nh.subscribe ("can_device1_can1_receive", 1000,Radar208_extract);
